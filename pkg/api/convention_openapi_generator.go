...
func (g *ConventionOpenAPIGenerator) buildConventionOperation(route *RouteInfo, components *Components) *Operation {
	...
	// Attach rule metadata (x-rules) using doc extractor
	g.addRulesExtension(route, operation)
...
// addRulesExtension scans request struct fields for `rule` tags and attaches
// documented rule metadata under x-rules. It uses DocExtractor to pull
// descriptions from functions whose names match rule identifiers.
func (g *ConventionOpenAPIGenerator) addRulesExtension(route *RouteInfo, operation *Operation) {
	if route == nil || operation == nil || g.extractor == nil {
		return
	}
	...
	entries := g.collectRuleEntriesFromType(reqType)
	if len(entries) == 0 {
		return
	}
	...
	operation.Extensions["x-rules"] = entries
	operation.XRules = entries
}

func (g *ConventionOpenAPIGenerator) collectRuleEntriesFromType(t reflect.Type) []map[string]interface{} {
	...
	walk = func(tt reflect.Type) {
		...
		for i := 0; i < tt.NumField(); i++ {
			...
			if ruleTag := f.Tag.Get("rule"); ruleTag != "" {
				entries := g.entriesFromRuleTag(ruleTag)
				if len(entries) > 0 {
...
func (g *ConventionOpenAPIGenerator) entriesFromRuleTag(ruleTag string) []map[string]interface{} {
	invs, err := parseRuleInvocations(ruleTag)
	if err != nil || len(invs) == 0 {
		return nil
	}
	entries := make([]map[string]interface{}, 0, len(invs))
	for _, inv := range invs {
		if inv.Name == "" {
			continue
		}
		entries = append(entries, g.buildRuleEntry(inv.Name))
	}
	return entries
}

func (g *ConventionOpenAPIGenerator) buildRuleEntry(name string) map[string]interface{} {
	entry := map[string]interface{}{"name": name}
	if name == "" || g.extractor == nil {
		return entry
	}
	if doc := g.extractor.ExtractFunctionDoc(name); doc.Description != "" {
		entry["description"] = doc.Description
	}
	return entry
}

// parseRuleInvocations extracts rule names from a rule tag string.
func parseRuleInvocations(s string) ([]struct{ Name string }, error) {
	s = strings.TrimSpace(s)
	if s == "" {
		return nil, nil
	}
	parts, err := splitTopLevelLocal(s, ',')
	if err != nil {
		return nil, err
	}
	invs := make([]struct{ Name string }, 0, len(parts))
	for _, p := range parts {
		...
		// Keep even empty names; filtering happens in entriesFromRuleTag
		invs = append(invs, struct{ Name string }{Name: name})
	}
	return invs, nil
}

// splitTopLevelLocal splits by sep while respecting parentheses and quotes.
func splitTopLevelLocal(s string, sep rune) ([]string, error) {
	var (
		parts    []string
		cur      strings.Builder
		depth    int
	...
	for _, r := range s {
		switch r {
		case '\'':
			handleSingleQuoteLocal(&inSingle, inDouble)
			cur.WriteRune(r)
		case '"':
			handleDoubleQuoteLocal(&inDouble, inSingle)
			cur.WriteRune(r)
		case '(':
			handleOpenParenLocal(&depth, inSingle, inDouble)
			cur.WriteRune(r)
		case ')':
			if err := handleCloseParenLocal(&depth, inSingle, inDouble); err != nil {
				return nil, err
			}
			cur.WriteRune(r)
		default:
			if shouldSplitLocal(r, depth, inSingle, inDouble, sep) {
				pushPartLocal(&parts, &cur)
				continue
			}
			cur.WriteRune(r)
		}
...
func handleSingleQuoteLocal(inSingle *bool, inDouble bool) {
	if !inDouble {
		*inSingle = !*inSingle
	}
}

func handleDoubleQuoteLocal(inDouble *bool, inSingle bool) {
	if !inSingle {
		*inDouble = !*inDouble
	}
}

func handleOpenParenLocal(depth *int, inSingle, inDouble bool) {
	if !inSingle && !inDouble {
		(*depth)++
	}
}

func handleCloseParenLocal(depth *int, inSingle, inDouble bool) error {
	if !inSingle && !inDouble {
		if *depth == 0 {
...
func shouldSplitLocal(r rune, depth int, inSingle, inDouble bool, sep rune) bool {
	return r == sep && depth == 0 && !inSingle && !inDouble
}

func pushPartLocal(parts *[]string, cur *strings.Builder) {
